...
void UPaintForgeGameInstance::Init()
{
	Super::Init();
	LoadOrCreateIdentity();
}

FGuid UPaintForgeGameInstance::GetLocalPlayerGuid() const
{
	return LocalPlayerGuid;
}

FString UPaintForgeGameInstance::GetLocalPlayerGuidHash() const
{
	return LocalPlayerGuidHash;
}

FString UPaintForgeGameInstance::GetIdentityFilePath() const
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("PaintForge"), TEXT("Identity.json"));
}

void UPaintForgeGameInstance::LoadOrCreateIdentity()
{
	...
	UE_LOG(PaintForgeLog, Log, TEXT("Install identity ready (hash %s...)"), *LocalPlayerGuidHash.Left(8));
}
