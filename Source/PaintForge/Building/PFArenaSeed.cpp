...
namespace
{
		...
		const TArray<FPFBuildPieceRec>& Pieces)
	{
		...
		{
			UE_LOG(PaintForgeLog, Error, TEXT("ArenaSeed: failed to write %s"), *Path);
			return false;
		}
		UE_LOG(PaintForgeLog, Log, TEXT("ArenaSeed: wrote %s (%d pieces)"), *FileName, Pieces.Num());
		return true;
	}
...
int32 FPFArenaSeed::EnsureSeedArenas()
{
	...
	if (Written > 0)
	{
		UE_LOG(PaintForgeLog, Log, TEXT("ArenaSeed: created %d seed community map(s)"), Written);
	}
	return Written;
}
