...
void UPFRootHUDWidget::NativeConstruct()
{
	...
	WirePawn(Cast<APaintForgeCharacter>(GetOwningPlayerPawn()));
...
void UPFRootHUDWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	APaintForgeCharacter* CurrentPawn = Cast<APaintForgeCharacter>(GetOwningPlayerPawn());
...
APaintForgePlayerController* UPFRootHUDWidget::GetPFPlayerController() const
{
	return Cast<APaintForgePlayerController>(GetOwningPlayer());
}
...
void UPFRootHUDWidget::BindOwningPC()
{
	...
	APaintForgePlayerController* PC = GetPFPlayerController();
	if (!PC)
	{
		return;
	}
...
{
	if (APaintForgePlayerController* PC = GetPFPlayerController())
	{
...
{
	WirePawn(Cast<APaintForgeCharacter>(NewPawn));
}

void UPFRootHUDWidget::WirePawn(APaintForgeCharacter* NewPawn)
{
	if (BoundBuild.IsValid())
	{
		BoundBuild->OnBuildWheelRequestedEvent.RemoveAll(this);
...
void UPFRootHUDWidget::UpdateScoreboardVisibility()
{
	...
	const APaintForgePlayerController* PC = GetPFPlayerController();
	const APaintForgeGameState* GS = BoundGameState.Get();
